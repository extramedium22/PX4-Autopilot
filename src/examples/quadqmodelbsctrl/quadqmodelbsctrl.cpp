...
extern "C" __EXPORT int quadcopter_qmodel_backstepping_ctrl(int argc, char *argv[]);
...
extern "C" __EXPORT int sign_ck(float input);

extern "C" __EXPORT int quadqmodelbsctrl_main(int argc, char *argv[]);
int quadqmodelbsctrl_main(int argc, char *argv[])
{
	...
	if (!strcmp(argv[1], "start")) {
		...
		daemon_task = px4_task_spawn_cmd("quadcopterbsctrl",
						 ...
						 quadcopter_qmodel_backstepping_ctrl,
...
int quadcopter_qmodel_backstepping_ctrl(int argc, char *argv[])
{
		...
		if (poll_ret == 0) {
			...
			*/
			...
			if (fds[0].revents & POLLIN) {
				...
				orb_copy(ORB_ID(vehicle_attitude), att_sub_fd, &current_att);
				current_status.q0 = current_att.q[0];
				current_status.q1 = current_att.q[1];
				current_status.q2 = current_att.q[2];
				current_status.q3 = current_att.q[3];
				current_status.phi = atan2(2 * (current_status.q0 * current_status.q1 + current_status.q2 * current_status.q3), 1 - 2 * (current_status.q1 * current_status.q1 + current_status.q2 * current_status.q2));
				current_status.theta = asin(2 * (current_status.q0 * current_status.q2 - current_status.q1 * current_status.q3));
				current_status.psi = atan2(2 * (current_status.q0 * current_status.q3 + current_status.q1 * current_status.q2), 1 - 2 * (current_status.q2 * current_status.q2 + current_status.q3 * current_status.q3));
				orb_copy(ORB_ID(vehicle_angular_velocity), attvel_sub_fd, &current_attvel);
				current_status.p = current_attvel.xyz[0];
				current_status.q = current_attvel.xyz[1];
				current_status.r = current_attvel.xyz[2];
				...
				*/
				...
				//for testing
				//rotor_commands.control[0] = 0.5;
				//rotor_commands.control[1] = 0.5;
				//rotor_commands.control[2] = 0.5;
				//rotor_commands.control[3] = 0.5;
				//rotor_commands.control[4] = 0.0;
				//rotor_commands.control[5] = 0.0;
				//rotor_commands.control[6] = 0.0;
				//rotor_commands.control[7] = 0.0;
...
struct ud_vec desired_control_law(struct status current_status, struct pos_vec target_pos, struct pos_vec target_vel, float target_yaw, float target_yawspeed)
{
	...
	float dot_A_1_inv_11 = 0;
	float dot_A_1_inv_12 = 0;
	float dot_A_1_inv_13 = 0;
	float dot_A_1_inv_21 = 0;
	float dot_A_1_inv_22 = 0;
	float dot_A_1_inv_23 = 0;
	float dot_A_1_inv_31 = 0;
	float dot_A_1_inv_32 = 0;
	float dot_A_1_inv_33 = 0;

	float f_scale_phi = 1;
	float f_scale_theta = 1;
	float f_scale = 1;
	...
	*/
	...
	// from n3d to qd
	phid = (float)asin(n3xd * (float)sin(current_status.psi) - n3yd * (float)cos(current_status.psi));
	thetad = (float)asin((n3xd * (float)cos(current_status.psi) + n3yd * (float)sin(current_status.psi)) / (float)cos(current_status.phi));
	if (phid > (float)0.785)
	{
		phid = 0.785;
	}
	else if (phid < (float)-0.785)
	{
		phid = -0.785;
	}
	if (thetad > (float)0.785)
	{
		thetad = 0.785;
	}
	else if (thetad < (float)-0.785)
	{
		thetad = -0.785;
	}
	r11 = (float)cos(psid) * (float)cos(thetad);
	r12 = (float)cos(psid) * (float)sin(thetad) * (float)sin(phid) - (float)sin(psid) * (float)cos(phid);
	r13 = (float)cos(psid) * (float)sin(thetad) * (float)cos(phid) + (float)sin(psid) * (float)sin(phid);
	r21 = (float)sin(psid) * (float)cos(thetad);
	r22 = (float)sin(psid) * (float)sin(thetad) * (float)sin(phid) + (float)cos(psid) * (float)cos(phid);
	r23 = (float)sin(psid) * (float)sin(thetad) * (float)cos(phid) - (float)cos(psid) * (float)sin(phid);
	r31 = -1 * (float)sin(thetad);
	r32 = (float)cos(thetad) * (float)sin(phid);
	r33 = (float)cos(thetad) * (float)cos(phid);

	//q0d = 0.5 * sqrt(1 + r11 + r22 + r33);
	q1d = sign_ck(r32 - r23) * (float)0.5 * (float)sqrt(1 + r11 - r22 - r33);
	q2d = sign_ck(r13 - r31) * (float)0.5 * (float)sqrt(1 - r11 + r22 - r33);
	q3d = sign_ck(r21 - r12) * (float)0.5 * (float)sqrt(1 - r11 - r22 + r33);
	...
	sq1 = current_status.q1 - q1d;
	sq2 = current_status.q2 - q2d;
	sq3 = current_status.q3 - q3d;

	A_1_inv_11 = (float)0.5 * (current_status.q0 * current_status.q0 + current_status.q1 * current_status.q1) / current_status.q0;
	A_1_inv_12 = (float)0.5 * (current_status.q0 * current_status.q3 + current_status.q1 * current_status.q2) / current_status.q0;
	A_1_inv_13 = (float)0.5 * (current_status.q1 * current_status.q3 - current_status.q0 * current_status.q2) / current_status.q0;
	A_1_inv_21 = (float)0.5 * (current_status.q1 * current_status.q2 - current_status.q0 * current_status.q3) / current_status.q0;
	A_1_inv_22 = (float)0.5 * (current_status.q0 * current_status.q0 + current_status.q2 * current_status.q2) / current_status.q0;
	A_1_inv_23 = (float)0.5 * (current_status.q0 * current_status.q1 + current_status.q2 * current_status.q3) / current_status.q0;
	A_1_inv_31 = (float)0.5 * (current_status.q0 * current_status.q2 + current_status.q1 * current_status.q3) / current_status.q0;
	A_1_inv_32 = (float)0.5 * (current_status.q2 * current_status.q3 - current_status.q0 * current_status.q1) / current_status.q0;
	A_1_inv_33 = (float)0.5 * (current_status.q0 * current_status.q0 + current_status.q3 * current_status.q3) / current_status.q0;

	//PX4_INFO("sn3 = %.4f, %.4f", (double)sn3x, (double)sn3y);

	pd = -1 * kn3 * (A_1_inv_11 * sq1 + A_1_inv_12 * sq2 + A_1_inv_13 * sq3);
	qd = -1 * kn3 * (A_1_inv_21 * sq1 + A_1_inv_22 * sq2 + A_1_inv_23 * sq3);
	rd = -1 * kpsi * (A_1_inv_31 * sq1 + A_1_inv_32 * sq2 + A_1_inv_33 * sq3);
	sp = current_status.p - pd;
	sq = current_status.q - qd;
	sr = current_status.r - rd;

	dq0 = (float)0.5 * (-1 * current_status.p * current_status.q1 - current_status.q * current_status.q2 - current_status.r * current_status.q3);
	dq1 = (float)0.5 * (current_status.p * current_status.q0 - current_status.q * current_status.q3 + current_status.r * current_status.q2);
	dq2 = (float)0.5 * (current_status.p * current_status.q3 + current_status.q * current_status.q0 - current_status.r * current_status.q1);
	dq3 = (float)0.5 * (-1 * current_status.p * current_status.q2 + current_status.q * current_status.q1 + current_status.r * current_status.q0);

	dot_A_1_inv_11 = (float)0.5 * (dq0 * (current_status.q0 * current_status.q0 - current_status.q1 * current_status.q1)+ 2 * dq1 * current_status.q0 * current_status.q1) / current_status.q0 * current_status.q0;
	dot_A_1_inv_12 = (float)0.5 * ((current_status.q0 * dq3 + dq1 * current_status.q2 + current_status.q1 * dq2) * current_status.q0 - current_status.q1 * current_status.q2 * dq0) / current_status.q0 * current_status.q0;
	dot_A_1_inv_13 = (float)0.5 * (-1 * ((current_status.q0 * dq2 - dq1 * current_status.q3 - current_status.q1 * dq3) * current_status.q0 + current_status.q1 * current_status.q3 * dq0)) / current_status.q0 * current_status.q0;
	dot_A_1_inv_21 = (float)0.5 * (-1 * ((current_status.q0 * dq3 - dq1 * current_status.q2 - current_status.q1 * dq2) * current_status.q0 + current_status.q1 * current_status.q2 * dq0)) / current_status.q0 * current_status.q0;
	dot_A_1_inv_22 = (float)0.5 * (dq0 * (current_status.q0 * current_status.q0 - current_status.q2 * current_status.q2)+ 2 * dq2 * current_status.q0 * current_status.q2) / current_status.q0 * current_status.q0;
	dot_A_1_inv_23 = (float)0.5 * ((current_status.q0 * dq1 + dq2 * current_status.q3 + current_status.q2 * dq3) * current_status.q0 - current_status.q2 * current_status.q3 * dq0) / current_status.q0 * current_status.q0;
	dot_A_1_inv_31 = (float)0.5 * ((current_status.q0 * dq2 + dq1 * current_status.q3 + current_status.q1 * dq3) * current_status.q0 - current_status.q1 * current_status.q3 * dq0) / current_status.q0 * current_status.q0;
	dot_A_1_inv_32 = (float)0.5 * (-1 * ((current_status.q0 * dq1 - dq2 * current_status.q3 - current_status.q2 * dq3) * current_status.q0 + current_status.q2 * current_status.q3 * dq0)) / current_status.q0 * current_status.q0;
	dot_A_1_inv_33 = (float)0.5 * (dq0 * (current_status.q0 * current_status.q0 - current_status.q3 * current_status.q3)+ 2 * dq3 * current_status.q0 * current_status.q3) / current_status.q0 * current_status.q0;
	...
	ud.taop = Jx * (-1 * kw * sp - (float)0.5 * (current_status.q0 * sq1 + current_status.q3 * sq2 - current_status.q2 * sq3) - ((Jy - Jz) * current_status.q * current_status.r / Jx) - kn3 * (dot_A_1_inv_11 * sq1 + dot_A_1_inv_12 * sq2 + dot_A_1_inv_13 * sq3) - kn3 * current_status.p);
	ud.taoq = Jy * (-1 * kw * sq - (float)0.5 * (-1 * current_status.q3 * sq1 + current_status.q0 * sq2 + current_status.q1 * sq3) - ((Jz - Jx) * current_status.p * current_status.r / Jy) - kn3 * (dot_A_1_inv_21 * sq1 + dot_A_1_inv_22 * sq2 + dot_A_1_inv_23 * sq3) - kn3 * current_status.q);
	ud.taor = Jz * (-1 * kwz * sr - (float)0.5 * (current_status.q2 * sq1 - current_status.q1 * sq2 + current_status.q0 * sq3) - ((Jx - Jy) * current_status.p * current_status.q / Jz) - kpsi * (dot_A_1_inv_31 * sq1 + dot_A_1_inv_32 * sq2 + dot_A_1_inv_33 * sq3) - kpsi * current_status.r);
	...
	*/
	...
	f_scale_phi = (-1 / (1 + (float)exp(-100 * (((float)abs(current_status.phi) - (float)0.5))))) + 1;
	f_scale_theta = (-1 / (1 + (float)exp(-100 * (((float)abs(current_status.theta) - (float)0.5))))) + 1;

	f_scale = f_scale_phi * f_scale_theta;
