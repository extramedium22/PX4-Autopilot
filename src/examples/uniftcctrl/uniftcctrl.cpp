...
extern "C" __EXPORT int uniform_ftc_ctrl(int argc, char *argv[]);

//extern "C" __EXPORT struct T_vec control_allocation(struct ud_vec ud);
extern "C" __EXPORT struct T_vec control_allocation(float H[4][4], float WM[4][4], struct ud_vec uc, struct T_vec T0);

extern "C" __EXPORT struct ud_vec desired_control_law(struct status current_status, struct pos_vec target_pos, struct pos_vec target_vel, float target_yaw, float target_yawspeed, struct T_vec T_k1);

//extern "C" __EXPORT struct ud_vec u_filter(struct ud_vec u_in, struct ud_vec u_last);

extern "C" __EXPORT int uniftcctrl_main(int argc, char *argv[]);
int uniftcctrl_main(int argc, char *argv[])
{

	if (argc < 2) {
		PX4_WARN("usage: uniftcctrl {start}\n");
		return 1;
	}

	if (!strcmp(argv[1], "start")) {

		daemon_task = px4_task_spawn_cmd("uniftcctrl",
						 SCHED_DEFAULT,
...
int uniform_ftc_ctrl(int argc, char *argv[])
{
		...
		if (poll_ret == 0) {
			...
			*/
			...
			if (fds[0].revents & POLLIN) {
				...
				q2 = current_att.q[2];
				q3 = current_att.q[3];
				current_status.phi = (float)atan2(2 * (q0 *q1 + q2 * q3), 1 - 2 * (q1 * q1 + q2 * q2));
				current_status.theta = (float)asin(2 * (q0 * q2 - q1 * q3));
				current_status.psi = (float)atan2(2 * (q0 * q3 + q1 * q2), 1 - 2 * (q2 * q2 + q3 * q3));
				orb_copy(ORB_ID(vehicle_angular_velocity), attvel_sub_fd, &current_attvel);
				current_status.p = current_attvel.xyz[0];
				current_status.q = current_attvel.xyz[1];
				current_status.r = current_attvel.xyz[2];
				...
				*/
				...
				rotor_commands.control[0] = ((float)sqrt(T_desired.T1/mc) / maxRotVelocity) * (float)T_eff.T1;
				rotor_commands.control[1] = ((float)sqrt(T_desired.T2/mc) / maxRotVelocity) * (float)T_eff.T2;
				rotor_commands.control[2] = ((float)sqrt(T_desired.T3/mc) / maxRotVelocity) * (float)T_eff.T3;
				rotor_commands.control[3] = ((float)sqrt(T_desired.T4/mc) / maxRotVelocity) * (float)T_eff.T4;
				...
				//rotor_commands.control[0] = 0.1;
				//rotor_commands.control[1] = 0.1;
				//rotor_commands.control[2] = 0.1;
				//rotor_commands.control[3] = 0.1;
