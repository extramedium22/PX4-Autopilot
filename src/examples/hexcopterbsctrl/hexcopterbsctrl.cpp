...
extern "C" __EXPORT int hexcopter_backstepping_ctrl(int argc, char *argv[]);
...
extern "C" __EXPORT int hexcopterbsctrl_main(int argc, char *argv[]);
int hexcopterbsctrl_main(int argc, char *argv[])
{

	if (argc < 2) {
		PX4_WARN("usage: hexcopterbsctrl {start}\n");
		return 1;
	}

	if (!strcmp(argv[1], "start")) {

		daemon_task = px4_task_spawn_cmd("hexcopterbsctrl",
						 SCHED_DEFAULT,
						 SCHED_PRIORITY_MAX - 5,
						 2000,
						 hexcopter_backstepping_ctrl,
...
int hexcopter_backstepping_ctrl(int argc, char *argv[])
{
			...
			*/
			...
			if (fds[0].revents & POLLIN) {
				...
				*/
				...
				rotor_commands.control[0] = ((float)0.001 * ((float)sqrt(T_desired.T4/mc)));
				rotor_commands.control[1] = ((float)0.001 * ((float)sqrt(T_desired.T4/mc)));
				rotor_commands.control[2] = ((float)0.001 * ((float)sqrt(T_desired.T2/mc)));
				rotor_commands.control[3] = ((float)0.001 * ((float)sqrt(T_desired.T3/mc)));
				rotor_commands.control[4] = ((float)0.001 * ((float)sqrt(T_desired.T3/mc)));
				rotor_commands.control[5] = ((float)0.001 * ((float)sqrt(T_desired.T1/mc)));
				rotor_commands.control[6] = 0.0;
				rotor_commands.control[7] = 0.0;
				rotor_commands.control[8] = 0.0;
				rotor_commands.control[9] = 0.0;
...
struct T_vec control_allocation(struct ud_vec uc)
{
	...
	T[0] = uc.f * (float)0.1922 + uc.taop * (float)1.0373 + uc.taoq * (float)1.7007 - uc.taor * (float)30.3942;
	T[1] = uc.f * (float)0.1922 - uc.taop * (float)1.0373 + uc.taoq * (float)1.7007 + uc.taor * (float)30.3942;
	T[2] = uc.f * (float)0.1539 + uc.taop * (float)0.5187 - uc.taoq * (float)0.8503 + uc.taor * (float)9.8029;
	T[3] = uc.f * (float)0.1539 - uc.taop * (float)0.5187 - uc.taoq * (float)0.8503 - uc.taor * (float)9.8029;
